...
exports.getPlaylistFromMix = function(mixID,callback){
    var numSongs;
    var streamKeys = [];
    var echonestCallback = function(playlist) {
    ...
    var groovesharkSongQueryCallback = function(queryResult) {
        ...
        if(queryResult.result.songs.length > 0) {
            ...
            gs.make_request(streamQuery, function (result) {
                result.songName = queryResult.result.SongName;
                result.artistName = queryResult.result.ArtistName;
    ...
    echo.getPlaylistFromMix(mixID,echonestCallback);
    //echo.getPlaylistFromMix("5132d5b6e85596b332000005",echonestCallback);
};